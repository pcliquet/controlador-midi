...
static void task_adc(void *pvParameters) {
	for (;;) {
		uint32_t msg = 0;
		if (xQueueReceive(xQueueMEDIA, &msg, 1000)) {
			printf("A media recebida e de %d \n",msg);
		}
		delay_ms(1000);
	}
}
...
static void task_proc(void *pvParameters) {
  ...
  while (1) {
	 if (n==10){
		 uint32_t media = soma/n;
		 printf("Enviou %d \n",media);
		 xQueueSend(xQueueMEDIA, (void *)&media, 10);
		 n = 0;
		 soma = 0;
	 } 
	else{	
...
int main(void) {
  ...
  xQueueMEDIA = xQueueCreate(100, sizeof(uint32_t));
  if (xQueueADC == NULL)
